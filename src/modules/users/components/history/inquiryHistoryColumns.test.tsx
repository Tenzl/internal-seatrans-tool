...
describe('shipping agency inquiry party columns', () => {
  it('shows employee but not client-submitter on the admin shipping list', () => {
    ...
    expect(adminColumns.map((column) => column.id)).toContain(
      'employeeInCharge'
    )
    expect(adminColumns.map((column) => column.id)).not.toContain(
      'clientSubmittedBy'
    )
