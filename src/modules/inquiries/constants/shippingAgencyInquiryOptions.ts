...
const normalizePurposeKey = (value: string) =>
  value
    .trim()
    .toUpperCase()
    .replace(/[\s-]+/g, '_')

export function formatPurposeOfCalling(
  value: string | null | undefined
): string {
  if (!value?.trim()) return ''
  const normalized = normalizePurposeKey(value)
  const match = PURPOSE_OF_CALLING_OPTIONS.find(
    (option) => option.value === normalized
  )
  return match?.label ?? value
}

export function formatPublicFrtTaxType(
  value: string | null | undefined
): string {
  if (!value?.trim()) return ''
