...
vi.mock('@/modules/inquiries/services/epdaParametersService', () => ({
...
describe('resolveEffectiveParams', () => {
  ...
  it('uses the canonical port override despite an exact-name miss and preserves the HN area', async () => {
    await resolveEffectiveParams(
      'HN',
      'legacy name that does not match the catalog',
      38
    )
  ...
  it('surfaces a canonical port lookup failure instead of masking it with worksheet defaults', async () => {
    getEffective.mockRejectedValueOnce(
      new Error('Port 38 is not assigned to an EPDA area')
    )

    await expect(
      resolveEffectiveParams('HCM', 'legacy mismatched name', 38)
    ).rejects.toThrow('Port 38 is not assigned to an EPDA area')
  ...
  it('keeps name lookup as a fallback for legacy inquiries without a port id', async () => {
    getPortsByArea.mockResolvedValue([
      { id: 21, name: 'Quy Nhon', portOfCall: 'QUY NHON' },
    ])
