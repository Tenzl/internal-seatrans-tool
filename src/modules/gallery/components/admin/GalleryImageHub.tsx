...
const WorkspaceAside = memo(function WorkspaceAside({
  ...
  return (
    <aside className='shrink-0 border-b border-border/60 bg-muted/20 lg:w-[15.5rem] lg:border-r lg:border-b-0'>
      <div className='px-4 py-4 lg:px-5 lg:py-6'>
        <p className='mb-3 text-[0.65rem] font-semibold tracking-[0.08em] text-muted-foreground uppercase lg:mb-4'>
          Workspace
        </p>
      ...
      <div className='flex min-h-0 min-w-0 flex-1 flex-col bg-background/80'>
        <header className='border-b border-border/60 px-4 py-4 md:px-7 md:py-5'>
          <div className='flex flex-col gap-3 md:flex-row md:items-start md:justify-between'>
            <p className='max-w-2xl min-w-0 text-sm leading-relaxed text-muted-foreground'>
        ...
        <div className='flex-1 overflow-y-auto px-4 py-5 md:px-7 md:py-6'>
          <AnimatePresence mode='wait'>
            <motion.div key={activeTab} {...TAB_MOTION}>
              {activeTab === 'add' ? (
                <AddImageTab onUploadSuccess={() => onSelectTab('manage')} />
              ) : (
...
export function GalleryImageHub() {
  ...
  const activeMode =
    MODE_OPTIONS.find((m) => m.id === activeTab) ?? MODE_OPTIONS[0]
  ...
  return (
    <div className='relative flex min-h-0 flex-col overflow-hidden'>
      <div
        className='maritime-grid-subtle pointer-events-none absolute inset-0 opacity-[0.35]'
        aria-hidden
      />
