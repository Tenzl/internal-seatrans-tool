...
export function EpdaFormSection({
  id,
  title,
  description,
  children,
  ...
  return (
    <section
      ...
      className={cn(
        ...
        railMode
          ? 'border-t-0 pt-0'
          : 'border-t border-border/50 pt-9 first:border-t-0 first:pt-0',
        isHidden && 'hidden',
        className
      )}
    >
      <div className='flex items-start gap-3'>
        {stepNumber ? (
          <span
            aria-hidden
            className='mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-lg font-semibold text-primary tabular-nums'
          >
            {stepNumber}
          </span>
        ) : null}
